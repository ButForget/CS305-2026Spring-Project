...
The DHCP test suite validates that the controller's built-in DHCP server correctly allocates IP addresses to hosts, handling the complete DHCP lifecycle across both interactive and CI test files:

\begin{itemize}[leftmargin=1.5cm]
    \item \textbf{Basic DHCP (\texttt{test\_network.py}, \texttt{test\_dhcp.py}):} Hosts broadcast a DHCP DISCOVER message and receive a valid OFFER with an IP from the configured pool. Hosts confirm the offered address via REQUEST and receive an ACK with correct lease parameters. Multiple hosts receive distinct IP addresses with no conflicts, and all allocated addresses fall within the pool range.
    \item \textbf{DHCP RELEASE (\texttt{test\_network.py}, \texttt{test\_dhcp.py}):} (Bonus) A host releases its DHCP lease; the IP is returned to the pool and can be re-issued to another host. The interactive test uses pool exhaustion (requires a pool size $\leq$ number of hosts) to prove that a released IP is truly reclaimed.
    \item \textbf{Duplicate IP Rejection (\texttt{test\_network.py}, \texttt{test\_dhcp.py}):} (Bonus) A host attempts to steal an IP that is already bound to another host. The server responds with DHCPNAK and either assigns a different IP or leaves the client without a valid address.
...
The interactive test uses a single-switch topology with 3 hosts (to enable pool exhaustion in the release test when the pool is small). The CI test uses a single-switch topology with 2 hosts.
...
\midrule
\texttt{test\_basic\_dhcp} & Two hosts each get a valid, unique pool IP & \texttt{test\_dhcp.py} (CI), \texttt{test\_network.py} (3 hosts) \\
\texttt{test\_dhcp\_release} & Released IP is returned to pool and re-issued & \texttt{test\_dhcp.py} (CI), \texttt{test\_network.py} \\
\texttt{test\_duplicate\_ip} & Server sends NAK for stolen IP; host gets different IP or no IP & \texttt{test\_dhcp.py} (CI), \texttt{test\_network.py} \\
