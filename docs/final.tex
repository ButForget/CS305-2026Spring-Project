...
\textbf Chenyu Yang(12413306) Xinyi Zhu (12412254) Jiachen Liu(12413307)
\end{center}
...
\midrule
\texttt{test\_baseline\_pingall} & All host pairs ping with 0\% loss after ARP learning & \texttt{test\_network.py} \\
\texttt{test\_flow\_table\_dump} & Flow entries printed per switch for manual inspection & \texttt{test\_network.py} \\
\texttt{test\_link\_failure\_single} & Single link down: connectivity maintained via alternate paths & \texttt{test\_network.py} \\
\texttt{test\_link\_failure\_double} & Double link down: connectivity maintained (mesh topologies) & \texttt{test\_network.py} \\
\texttt{test\_link\_failure\_isolate} & All links to one switch brought down: attached hosts isolated & \texttt{test\_network.py} \\
\texttt{test\_link\_no\_redundancy} & Link down in Line topology: expected partition & \texttt{test\_network.py} \\
\texttt{test\_port\_modify\_down} & OpenFlow port admin-down: rerouting or isolation verified & \texttt{test\_network.py} \\
\texttt{test\_port\_mod\_noforward} & OFPPC\_NO\_FWD blocks forwarding but not receiving & \texttt{test\_network.py} \\
\texttt{test\_port\_mod\_noreceive} & OFPPC\_NO\_RECV blocks receiving but not forwarding & \texttt{test\_network.py} \\
\texttt{test\_port\_modify\_combined} & Mixed port configs (down + no-forward) on multiple switches & \texttt{test\_network.py} \\
\texttt{test\_port\_flap} & Rapid port toggle (3 cycles): controller stabilises and recovers & \texttt{test\_network.py} \\
\texttt{test\_recovery} & All links/ports restored: full connectivity verified & \texttt{test\_network.py} \\
\midrule
\texttt{test\_complex\_baseline} & 28 host pairs ping with 0\% loss in HeavyMesh & \texttt{test\_network\_complex.py} \\
\texttt{test\_complex\_link\_single} & Single link failure in HeavyMesh: connectivity maintained & \texttt{test\_network\_complex.py} \\
\texttt{test\_complex\_link\_double} & Two simultaneous failures: still connected via cross-links & \texttt{test\_network\_complex.py} \\
\texttt{test\_complex\_link\_triple} & Triple failure isolates one switch: hosts under it are unreachable & \texttt{test\_network\_complex.py} \\
\texttt{test\_complex\_recovery} & All links restored: full 28-pair connectivity restored & \texttt{test\_network\_complex.py} \\
\bottomrule
\end{tabular}
...
The DHCP test suite validates the controller's built-in DHCP server across three interactive test files, covering the complete DHCP lifecycle --- from basic address allocation to edge cases such as pool exhaustion, duplicate defence, and lease timeouts:

\begin{itemize}[leftmargin=1.5cm]
    \item \textbf{Interactive Basic / Release / Duplicate Test (\texttt{test\_network.py}):} A single-switch, 3-host interactive test that exercises three distinct scenarios.\\
    \textbf{TC1 --- Basic DHCP:} All three hosts send DHCP DISCOVER and receive valid OFFERs with IP addresses from the configured pool; each host confirms via REQUEST/ACK and obtains a unique, in-pool IP.\\
    \textbf{TC2 (Bonus) --- DHCP RELEASE:} A host releases its lease via \texttt{dhclient -r}; the IP is returned to the pool and subsequently re-issued to another host. When the pool is small enough (e.g., 3 addresses for 3 hosts), the test uses pool exhaustion to prove reclamation by forcing the released IP to be the only available address. For larger pools, a lenient distinct-IP check is performed instead.\\
    \textbf{TC3 (Bonus) --- Duplicate IP Rejection:} Host h2 manually adopts h1's IP address before sending a REQUEST. The server detects that the requested IP is bound to a different MAC, responds with DHCPNAK, and h2 ultimately receives a different IP (or no IP), confirming that duplicate allocation is prevented per RFC 2131.
    \item \textbf{Overflow Test (\texttt{test\_network\_overflow.py}):} (Bonus) Deploys $m = n + \max(3, \lfloor n/3 \rfloor)$ hosts where $n$ is the pool size, guaranteeing more hosts than available IPs. All hosts run \texttt{dhclient} in parallel via \texttt{sendCmd}/\texttt{waitOutput}. The test verifies that exactly $n$ hosts receive valid in-pool addresses and the remaining $m-n$ hosts get no IP --- confirming that the server correctly rejects DISCOVERs (via NAK) when the pool is exhausted, rather than silently failing or crashing.
    \item \textbf{Lease Expiry Test (\texttt{test\_lease\_expiry.py}):} (Bonus) Validates that timed-out leases are automatically reclaimed by \texttt{\_expire\_leases()}. Requires pre-configuration: \texttt{end\_ip="192.168.1.3"} (2-IP pool) and \texttt{lease\_time=5} (5-second leases). Three hosts are created. Steps: (1) h1 and h2 exhaust the 2-IP pool; (2) h1's \texttt{dhclient} is killed and its IP stripped --- the lease remains active server-side; (3) h3 (a fresh observer) requests an IP immediately and is rejected because the pool is still exhausted; (4) after waiting $\texttt{lease\_time}+2$ seconds, h3 requests again and now succeeds, receiving h1's old IP --- proving that h1's expired lease was reclaimed and the same address was re-issued.
\end{itemize}
...
\paragraph{Overflow Topology (used by \texttt{test\_network\_overflow.py})}

Also a single-switch star topology, but with a dynamically computed number of hosts: $m = n + \max(3, \lfloor n/3 \rfloor)$, where $n$ is the IP pool size. For the default 9-IP pool (192.168.1.2--192.168.1.10), this produces $m = 12$ hosts, ensuring at least 3 hosts will fail to obtain an address. All hosts are connected to a single switch \texttt{s1} with \texttt{ip="no ip defined/8"}.

\paragraph{Lease Expiry Topology (used by \texttt{test\_lease\_expiry.py})}

Single-switch star with exactly 3 hosts (\texttt{h1}, \texttt{h2}, \texttt{h3}), each connected to switch \texttt{s1}. The test requires manual pre-configuration: \texttt{Config.end\_ip="192.168.1.3"} (2 IPs in pool) and \texttt{Config.lease\_time=5} (5-second lease). The smaller pool ensures exhaustion with only 2 hosts, and the short lease time makes the test practical to run.
