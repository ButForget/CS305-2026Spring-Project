...
|   |-- dhcp_test/
|   |   +-- test_network.py  # DHCP test script
|   |-- firewall_test/
...
\begin{lstlisting}[caption={Host Learning ¡ª DNS Registration}]
...
if dst_ip in (self.dns_server_ip, self.nat_external_ip) or \
   (NAT.is_internal(src_ip) and NAT.is_external(dst_ip)):
    ...
    return
# Known host ¡ª reply from cache; unknown ¡ª flood
...
\begin{itemize}[leftmargin=1.5cm]
    \item \textbf{Address Pool Management:} Maintains a pool of available IP addresses (192.168.1.2 through 192.168.1.100) using a deque structure for efficient allocation and release.
    ...
    \item \textbf{Duplicate Prevention:} Ensures no IP address is allocated to multiple clients simultaneously through careful tracking of offers and bindings.
    \item \textbf{Stale Offer Expiration:} Offers that are not followed by a REQUEST within 30 seconds are automatically expired and the address is returned to the pool.
    \item \textbf{NAK Handling:} Responds with DHCPNAK when a client requests an unavailable address (e.g., one already allocated to another client).
\end{itemize}
...
Address Pool & 192.168.1.2 -- 192.168.1.100 (99 addresses) \\
...
\begin{itemize}[leftmargin=1.5cm]
    \item \textbf{DISCOVER/OFFER:} Hosts broadcast a DHCP DISCOVER message and receive a valid OFFER with an IP from the configured pool.
    \item \textbf{REQUEST/ACK:} Hosts confirm the offered address via REQUEST and receive an ACK with correct lease parameters.
    \item \textbf{IP Uniqueness:} Multiple hosts receive distinct IP addresses with no conflicts.
    \item \textbf{Pool Range:} All allocated addresses fall within the configured pool (192.168.1.2--192.168.1.100).
\end{itemize}
...
\node[sw] (s1) at (2, 0) {s1};
\node[host] (h1) at (0.5, 1.5) {h1};
\node[host] (h2) at (3.5, 1.5) {h2};

\draw[link, dashed] (h1) -- (s1);
\draw[link, dashed] (h2) -- (s1);
\end{tikzpicture}
\caption{DHCP Test Topology --- single switch, 2 hosts}
\end{figure}
...
The firewall test suite validates that deny rules are correctly translated into drop flows and enforced at the data plane:

\begin{itemize}[leftmargin=1.5cm]
    \item \textbf{ICMP Filtering:} Verifies that ICMP traffic (ping) is blocked between hosts specified in the deny rules while remaining permitted between other host pairs.
    \item \textbf{TCP Port Filtering:} Verifies that TCP connections to blocked ports (e.g., port 80) are rejected while connections to allowed ports (e.g., port 8080) succeed.
    \item \textbf{Rule Selectivity:} Ensures that firewall rules only affect the traffic they are designed to match, without impacting unrelated flows.
\end{itemize}
...
\end{tikzpicture}
\caption{Firewall Test Topology --- single switch, 3 hosts}
\end{figure}
...
The DNS test suite validates the in-controller DNS resolution service:

\begin{itemize}[leftmargin=1.5cm]
    \item \textbf{A Record Resolution:} Querying \texttt{h2.local} from h1 returns the correct IP address assigned to h2.
    \item \textbf{NXDOMAIN:} Querying a non-existent hostname (\texttt{nonexistent.local}) returns the NXDOMAIN response code.
    \item \textbf{PTR Reverse Lookup:} Reverse-resolving an IP address returns the correct hostname.
    \item \textbf{Automatic Registration:} Hosts obtain their IPs via DHCP, and the controller automatically registers their hostnames in the DNS upon learning their ARP bindings.
\end{itemize}
...
\end{tikzpicture}
\caption{DNS Test Topology --- single switch, 2 hosts}
\end{figure}
...
The NAT test suite validates correct address translation between internal and external subnets:

\begin{itemize}[leftmargin=1.5cm]
    \item \textbf{Basic NAT Connectivity:} Internal host h1 (10.0.1.2) can successfully ping external host h2 (10.0.2.2) through the NAT.
    \item \textbf{SNAT Source Verification:} External host h2 observes the NAT external IP (10.0.2.100) as the packet source, not h1's real internal IP address.
    \item \textbf{Bidirectional Connectivity:} External host h2 can initiate communication to h1 through DNAT (by sending to 10.0.2.100).
    \item \textbf{TCP Through NAT:} A TCP connection (e.g., HTTP on port 80) from h1 to h2 is correctly translated in both directions, maintaining connection state.
\end{itemize}
...
\begin{tikzpicture}[
    ...
    host/.style={draw, rectangle, rounded corners, minimum size=0.7cm, font=\footnotesize},
    link/.style={thick},
    subnet/.style={draw, dashed, rounded corners, inner sep=0.4cm}
]
...
\end{tikzpicture}
\caption{NAT Test Topology --- 2 switches, 2 hosts in different subnets}
\end{figure}
